...
<body>
	<?php 
		...
		$query = "SELECT * FROM applications WHERE email IN (SELECT email FROM users WHERE department = '$department_loggedin');";
	...
	?>
	<nav class="navbar-expand-lg navbar navbar-dark bg-primary navbar-fixed-top">
    ...
    <table class="table" id="table">
		...
		<tbody>
		  		...
		  		for($i=0;$i<count($row);$i++){
					...
					<?php if ($row[$i]['application_status_hod']=='A') {?>
		  		...
		  		for($i=0;$i<count($row);$i++){
					...
					<td><?php echo $row[$i]["application_status_admin"];?></td>
					<?php if ($row[$i]['application_status_hod']=='A') {?>
					<td><input type="button" id="mybutton"  class="btn btn-success" value="Approved" disabled></td>
					  <?php  } else { ?>
					  <td><input type="button" class="btn btn-primary" id="mybutton" value="Approve" onclick="myFunction('<?php echo $id ?>');"></td>
						  <?php  }  ?>
...
function myFunction(id) {
	...
	{
		window.location = "http://localhost/Dashboard_file/WebDashboardHod/index.php?id=" + id;
	}
	...
	<!-- <div class="container"> 
		...
		<div class="p-3 mb-2 bg-primary text-white">
			 <p><strong>Admin Approval: </strong><input type="checkbox" name="admin" disabled> </p>
			 <p><strong>HOD Approval: </strong><input type="checkbox" name="hod"></p>
		</div>
